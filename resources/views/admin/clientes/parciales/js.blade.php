    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        btnMostrarFormulario.addEventListener('click', () => {
            cardFormulario.style.display = 'block';
              $('#formTitulo').text('Crear nuevo Cliente');
        });
        // Ocultar formulario al hacer clic 
        btnCancelar.addEventListener('click', () => {
            cardFormulario.style.display = 'none';
        });

        // abrir formulario para editar
        // Captura botón Editar
        $(document).on('click', '#btnEdit', function() {
            const id = $(this).data('id');
            const urlEdit = "{{ url('admin/clientes') }}/" + id;
        ...
        $('#formCliente').submit(function(e) {
            ...
            const id = $('#id').val();
            const url = id ? `{{ url('admin/clientes') }}/${id}` :
        ...
        $(document).on('click', '#btnDelete', function() {
            const id = $(this).data('id');
            // console.log("Eliminar carrera con ID:", id);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(url, {
                        ...
                        .catch(error => {
                            Swal.fire('Error', 'No se pudo eliminar a el cliente.', 'error');
                            console.error(error);
                        });
                }
            });
...
<script>
    ...
    $(document).on('click', '.btnEdit', function () {
        const id = $(this).data('id');
        const urlEdit = `/admin/clientes/${id}`;
        $.get(urlEdit, function (data) {
            $('#id').val(data.id);
            $('#nombre').val(data.nombre);
            $('#correo').val(data.correo);
    ...
    $('#formCliente').submit(function (e) {
        ...
        const id = $('#id').val();
        const url = id ? urlEdit : `{{ route('admin.clientes.store') }}`;
        const method = id ? 'PUT' : 'POST';
